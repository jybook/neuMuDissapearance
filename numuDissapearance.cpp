...
void codeTester(){
	...
	cout << "line 40 done" << endl;
	
	TH2 *dissapearanceProb = new TH2D("dissapearanceProb", "Dissapearance Chi-squareds in ICARUS; Sin^2(2 Theta); Delta m^2", xbins, 0.0, xmax, ybins, 0.0, ymax);
	cout << "line 43 done" << endl;
	
	double xres = xmax/xbins;
	double yres = ymax/ybins;
	cout << "line 47 done" << endl;
	cout << "start ";
	cout << "start2 ";
	...
	for(int i=0; i<xbins; i++){
		...
		for (int j=0; j<ybins; j++){
			...
			//Draw a contour at about 1.65
			if((chiSquared >= 1.6) && (chiSquared <= 1.7)){chiSquared = 999999;}
...
void calculateEventRate(double *eventRateArr, double *crossSecArr, double *fluxArr){
	for(int i=0; i<200; i++){
		...
		eventRateArr[i] = crossSecArr[i]*fluxArr[i]*(7.176*(100)*6);
	}
}
